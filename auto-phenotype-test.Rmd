...
```{r}
tf <- read_excel('data/Tf3.xlsx') %>%
  select(- c(no, bloodamount, bloodtype)) %>%
  distinct() # there are some duplicated rows in tf
skimr::skim(tf)

massive_trans <- tf %>% 
  arrange(`pt no`, opdate, oporder, Tftime) %>%
  mutate(Tftime = ymd_hms(paste0(Tftime, '00'))) %>% 
  add_count(`pt no`) %>%
  group_by(`pt no`, oporder) %>%
  mutate(difftrans = difftime(lead(Tftime, 2), Tftime,
                              units = 'mins'),
         massive_trans = difftrans < 60) %>%
  ungroup() %>% 
  tidyr::replace_na(list(massive_trans = FALSE)) %>% 
  group_by(`pt no`, massive_trans) %>% 
...
massive_trans %>% 
  ...
  select(fileid2 = `pt no`, masstf,
         correct_start_time, ekg_end_time) %>% 
  readr::write_csv('data/2020-01-19_auto-mass-trans-phenotype-test.csv')
```
