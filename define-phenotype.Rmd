...
manual_times_df <- manual_labs %>% 
  # mutate(manual_start_time = as_datetime(manual_start_time)) %>% 
...
```{r}
# check_start <- manual_labs %>%
#   left_join(pheno_df, by = 'fileid') %>%
#   select(-c(orout, opend)) %>%
#   mutate(start_mass_trans = as_datetime(start_mass_trans),
#          manual_start_time = as_datetime(manual_start_time)) %>%
#   mutate(disagree = (manual_start_time - start_mass_trans)/60)
...
why %>% 
  filter(
    fileid %in% c('C4_190823_074052', 'D1_190905_201426', 'D1_190923_095547', 'A5_180504_073943', 'C2_190905_073807')
  ) %>% 
  select(- c(conv_val, sep_val)) %>% 
  distinct() %>%
  DT::datatable()
...
Check special cases:
```{r}
pheno_df %>% filter(fileid %in% c("A1_190215_132805", "A5_180504_073943", "C2_190905_073807", "C4_190823_074052", "D1_190905_201426", "D1_170425_103512", "F4_180827_112738"))
```
