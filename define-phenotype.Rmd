...
all_times %>% 
  select(fileid, correct_start_time, ekg_end_time) %>% 
  distinct() %>% 
...
data.frame(unknown_patients = unknown_patients) %>% 
  readr::write_csv('data/unknown-patients.csv')

all_times %>% 
  select(fileid, correct_start_time, ekg_end_time) %>% 
  filter(!is.na(correct_start_time)) %>% 
  distinct() %>% 
  readr::write_csv('data/mass-trans-phenotype-case-only.csv')
